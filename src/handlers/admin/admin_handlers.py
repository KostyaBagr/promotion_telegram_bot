...
async def cancel_create_post(message: Message, state: FSMContext):
    if await is_admin(message.from_user.id):
        ...
        if curr_state is None:
            return
        await state.finish()
        await message.reply('Создание поста отменено')
...
async def create_post_start(message: Message):
    ...
    if await is_admin(message.from_user.id):
        ...
        await message.reply("Напишите имя для презентации", reply_markup=await kb.next_or_cancel_post())
...
async def get_post_name(message: Message, state: FSMContext):
    ...
    if await is_admin(message.from_user.id):
        ...
        await FSMPost.next()
        await message.reply("Напишите текст для презентации", reply_markup=await kb.next_or_cancel_post())
